...
import com.alibaba.otter.canal.client.adapter.tcp.config.MappingConfig.EtlSetting;
import com.alibaba.otter.canal.client.adapter.tcp.config.MappingConfig.TcpMapping;
...
public class TcpEtlService extends AbstractEtlService {

    private TcpTemplate tcpTemplate;
    private MappingConfig config;

    public TcpEtlService(TcpTemplate tcpTemplate, MappingConfig config) {
        super("tcp", config);
    ...
    public EtlResult importData(List<String> params) {
        ...
        TcpMapping tcpMapping = this.config.getTcpMapping();
        EtlSetting etlSetting = tcpMapping.getEtlSetting();
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("TCP 全量count sql : {}, values:{}", countSql, values);
            }
            ...
            Boolean result = executeSqlImport(dataSource, sql, values, tcpMapping, impCount, errMsg);
            if (result) {
                String s = String.format("TCP 数据全量导入完成, 总共: %s 条数据，导入成功: %s 条数据, 耗时: %s 毫秒", total, impCount.get(),
            ...
            logger.error(e.getMessage(), e);
            errMsg.add("TCP 数据导入异常 =>" + e.getMessage());
        }
    ...
    protected boolean executeSqlImport(DataSource ds, String sql, List<Object> values,
        ...
        EtlSetting etlSetting = ((TcpMapping) adapterMapping).getEtlSetting();

        if (logger.isInfoEnabled()) {
            logger.info("TCP 批量导入, sql:{}, values:{}", sql, values);
        }
            ...
            Util.sqlRS(ds, sql, values, rs -> {
                try {
                    while (rs.next()) {
                        ...
                        CompletableFuture<Boolean> future = this.tcpTemplate.runAsync(etlSetting.getDatabase(),
                                etlSetting.getTable(), "update", data);
                        futures.add(future);
                    }
                } catch (Exception e) {
                    logger.error(e.getMessage(), e);
                    errMsg.add("TCP etl failed! ==>" + e.getMessage());
