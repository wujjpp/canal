...
import com.alibaba.otter.canal.client.adapter.grpc.config.MappingConfigLoader;
import com.alibaba.otter.canal.client.adapter.grpc.config.MappingConfig.MonitorTable;
import com.alibaba.otter.canal.client.adapter.grpc.monitor.GrpcConfigMonitor;
import com.alibaba.otter.canal.client.adapter.grpc.support.GrpcTemplate;
import com.alibaba.otter.canal.client.adapter.grpc.service.GrpcEtlService;
import com.alibaba.otter.canal.client.adapter.grpc.service.GrpcSyncService;

@SPI("grpc")
public class GrpcAdapter implements OuterAdapter {

    private static Logger logger = LoggerFactory.getLogger(GrpcAdapter.class);
    ...
    private GrpcSyncService grpcSyncService;

    private GrpcConfigMonitor configMonitor;

    private Properties envProperties;

    public Map<String, MappingConfig> getGrpcMapping() {
    ...
    public void init(OuterAdapterConfig configuration, Properties envProperties) {
        if (logger.isDebugEnabled()) {
            logger.debug("init grpc adapter");
        ...
        try {
            ...
            Map<String, MappingConfig> grpcMappingTmp = MappingConfigLoader.load(envProperties);

            // 这边的key是grpc目录下的fileName
            grpcMappingTmp.forEach((key, config) -> {
            ...
            if (configMap != null) {
                ...
                configMap.values().forEach(config -> {
                    ...
                    if (matchGroup) {
                        ...
                        List<MonitorTable> monitorTables = config.getGrpcMapping().getMonitorTables();
                ...
                if (!configs.isEmpty()) {
                    configs.forEach(config -> grpcSyncService.sync(config, dml));
                }
            }
        }
    }
    ...
    public EtlResult etl(String task, List<String> params) {
        ...
        GrpcEtlService grpcEtlService = new GrpcEtlService(this.grpcTemplate, config);
        if (config != null) {
            return grpcEtlService.importData(params);
        } else {
            ...
            for (MappingConfig configTmp : this.mappingConfig.values()) {
                ...
                if (configTmp.getDestination().equals(task)) {
                    EtlResult etlRes = grpcEtlService.importData(params);
    ...
    public Map<String, Object> count(String task) {
        int count = this.grpcTemplate.count();
    ...
    public String getDestination(String task) {
        ...
        if (config != null && config.getGrpcMapping() != null) {
